...
export default async function CampeonatosPage() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-slate-950 text-white">
      ...
      <div className="fixed inset-0 -z-20 bg-gradient-to-b from-slate-950/35 via-slate-950/72 to-slate-950/95" />
              ...
              {campeonatos.length === 0 ? (
                ...
                <div className="grid gap-5 lg:grid-cols-2">
                  {campeonatos.map((campeonato) => (
                    <article
                      key={campeonato.id}
                      className="rounded-[2rem] border border-white/10 bg-slate-950/55 p-6 transition hover:-translate-y-1 hover:border-yellow-300/35"
                    >
